	...
	if(isset($_POST["paracall"])) {
		 ...
		 if ($_POST["paracall"] == "add_to") {
			...
			while( $row = mysqli_fetch_array($rst_ord_info) ){
				...
				$ord_arr[] = array(	"ord_id" => $ord_id, "prd" => $prd,
...
while($row=mysqli_fetch_array($get_product_dis_status))
{
    ...
    $sum+=$price*$qty;
        ...
        if (!$odr_dis == '') {
            $finall =($a34+ $odr_del_chrge + $odr_sr_chrge)-$odr_adv;
        }
        else{
            $finall = ($a34+ $odr_del_chrge + $odr_sr_chrge)-$odr_adv;
        }
        ...
        $resultasd = class_orders::insert_into_payments($inv_id,$cus_id,$pay_type,$odr_dis,$odr_adv,$odr_pay_bal,$odr_del_chrge,$odr_sr_chrge,$finall,$date);
        ...
        if ($result == 2) {
            ...
            echo '<script type="text/javascript">window.open("../invoice?inv_code='.$inv_id.'","newwindow","width=450,height=660");</script>';
