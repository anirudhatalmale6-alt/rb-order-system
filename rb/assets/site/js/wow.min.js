...
(function () {
    ...
    var TradingView = {
        ...
        generateUtmForUrlParams: function (options) {
            return "utm_source=" + encodeURI(window.location.hostname) + "&utm_medium=widget" + (options.type ? "&utm_campaign=" + options.type : "") + (options.type && options.type === "chart" ? "&utm_term=" + encodeURIComponent(options.symbol) : "")
        },
        ...
        QuotesProvider: function (options) {
            ...
            var options = {container: options.container_id, symbols: options.symbols || [], type: "quotes-provider"};
            ...
            function resolveSymbol(symbolObj) {
                ...
                postMessage.post(iframe.contentWindow, "resolveSymbol", {symbol: symbol});
                if (success) {
    ...
    TradingView.DependenciesManager.prototype.depends = function (scripts, cbid, cb) {
        for (var i = 0; i < scripts.length; i++) {
            if (this.scripts[scripts[i]] && this.scripts[scripts[i]].status === true) {
                ...
                this.scripts[scripts[i]] = {status: false, script: document.createElement("script"), callbacks: {}};
                this.scripts[scripts[i]].callbacks[cbid] = cb;
    ...
    TradingView.UserInfoWidget.prototype = {
        createWidget: function () {
            ...
            var query = TradingView.createUrlParams({username: this.options.username, locale: this.options.locale});
            this.options.type = "user-info";
            var utm = "&" + TradingView.generateUtmForUrlParams(this.options, {type: "UserInfoWidget"});
    ...
    TradingView.MediumWidget.prototype = {
        createWidget: function () {
            var widget_code = this.widgetCode();
            if (!this.options.noLogoOverlay && !this.options.whitelabel) {
                var greyText = this.options.greyText || "Quotes by";
                widget_code = TradingView.WidgetAbstract.prototype.addFooterLogo.call(this, widget_code, {
                    greyText: greyText,
                    linkText: "TradingView"
    ...
    TradingView.widget.prototype = {
        create: function () {
            ...
            addWidget(widget_html, this.options.container);
            if (!this.options.noLogoOverlay && this.options.fundamental) {
                var quotes = document.getElementById("tradingview-copyright");
                var tradingViewLink = '<a style="color: #3BB3E4">TradingView</a>';
                var linkText = this.options.copyright || "Fundamental data by {0}";
                if (quotes) {
                    quotes.parentNode.removeChild(quotes)
                }
                var iframe = document.getElementById(this.id);
                iframe.style.paddingBottom = "30px";
                iframe.style.boxSizing = "border-box";
                var tvLink = document.createElement("a");
                tvLink.setAttribute("href", "http://www.tradingview.com?" + TradingView.generateUtmForUrlParams(this.options));
                tvLink.setAttribute("rel", "nofollow");
                tvLink.setAttribute("target", "_blank");
                tvLink.style.fontFamily = "'Trebuchet MS', Tahoma, Arial, sans-serif";
                tvLink.style.fontSize = "13px";
                tvLink.style.color = "#ADAEB0";
                tvLink.innerHTML = linkText.replace("{0}", tradingViewLink);
                var newQuotes = document.createElement("span");
                newQuotes.style.display = "block";
                newQuotes.style.position = "absolute";
                newQuotes.style.bottom = "0";
                newQuotes.style.left = "0";
                newQuotes.style.width = "100%";
                newQuotes.style.height = "30px";
                newQuotes.style.lineHeight = "30px";
                newQuotes.style.textAlign = "center";
                newQuotes.appendChild(tvLink);
                iframe.parentNode.appendChild(newQuotes)
            }
            ...
            return '<iframe id="' + this.id + '"' + ' src="' + url + '"' + (this.options.autosize ? ' style="width: 100%; height: 100%; margin: 0 !important; padding: 0 !important;"' : ' width="' + this.options.width + '"' + ' height="' + this.options.height + '"') + ' frameborder="0" allowTransparency="true" scrolling="no" allowfullscreen></iframe>'
    ...
    TradingView.chart.prototype = {
        create: function () {
            ...
            this.postMessage = TradingView.postMessageWrapper(c.contentWindow, this.id)
        }, ready: TradingView.widget.prototype.ready, renderCss: function () {
            ...
            var actions = TradingView.gEl(this.id + "_actions");
    ...
    TradingView.showSignIn = function (options, callback) {
        TradingView.dependenciesManager.depends(["/static/lib/external/spin.min.js"], "authWidget", function () {
            ...
            var id = TradingView.gId();
            var query = TradingView.createUrlParams({id: id, utmSourceOverride: options.utmSourceOverride});
        ...
        return function (target, client_id) {
            ...
            return {
                on: function (name, callback, keepPrevious) {
                    ...
                    var msg = {id: post_id++, type: "post", name: name, data: data, provider: provider_id};
    ...
    TradingView.WidgetPopup.prototype = {
        create: function () {
            ...
            var left = (screen.width - width) / 2;
            var top = (screen.height - height) / 2;
            this.options.type = "popup";
            var url = TradingView.host + "/widgetpopup/" + "?symbol=" + encodeURIComponent(this.options.symbol) + "&interval=" + encodeURIComponent(this.options.interval) + "&toolbarbg=" + this.options.toolbar_bg.replace("#", "") + (this.options.theme ? "&theme=" + encodeURIComponent(this.options.theme) : "") + "&hidesidetoolbar=" + (this.options.hide_side_toolbar ? "1" : "0") + (this.options.locale ? "&locale=" + encodeURIComponent(this.options.locale) : "") + "&" + TradingView.generateUtmForUrlParams(this.options);
