...
{   
    ...
    public static function insert_into_invoice($inv_id="",$cus_id="",$rox_payment_id="",$date="",$d_delivery="",$time="",$tot="",$ord_by=""){
        ...
        $sql = "INSERT INTO rox_invoice (rox_inv_cus_id,rox_inv_auto_id,rox_inv_pay_id,rox_inv_date,rox_ord_time,rox_inv_time,rox_inv_due,rox_inv_by,rox_inv_status,rox_inv_balance,rox_del_date) VALUES
        ('$cus_id','$inv_id','$rox_payment_id','$dd','$ord_time','$time','$date','$ord_by','Pending','$tot','$d_delivery')";
		if (!mysqli_query($conn, $sql)) {
			return 1;
		} else {
			return 2;
    ...
    public  static function select_all_from_order($inv_code){
        include("library/dbcon.php");
        $sql1="SELECT * FROM rox_order_info WHERE rox_inv_id='$inv_code' ORDER BY `rox_prd_qty` DESC ";
        $result=mysqli_query($conn,$sql1);
        return $result;
        mysqli_close($conn);
    }
    public  static function select_all_from_order2($inv_code){
        include("library/dbcon.php");
        $sql1="SELECT * FROM rox_order_info WHERE rox_inv_id='$inv_code' ORDER BY `rox_discount_status` DESC ";
        $result=mysqli_query($conn,$sql1);
        return $result;
        mysqli_close($conn);
    }
    ...
    public  static function select_all_from_inv_without_condi(){
        include("library/dbcon.php");
        $sql1="SELECT * FROM rox_invoice ORDER BY rox_inv_id DESC limit 50";
        $result=mysqli_query($conn,$sql1);
        return $result;
        mysqli_close($conn);
    }
...
//    }
    ...
    public  static function seleect_all_from_odr_info($from_date=""){
        include("library/dbcon.php");
        $sql1="SELECT rox_line_id, rox_ord_id, rox_inv_id, rox_p_main_typ,rox_p_sub_type, rox_prd, rox_prd_val,rox_prd_price, rox_prd_qty,rox_gre_des,rox_gre,rox_gre_info, rox_gre_info2, rox_des,rox_ord_status,ord_date,GROUP_CONCAT(CONCAT_WS(' - ',rox_prd,rox_prd_qty ) separator ' <br> ') as rox_prd,sum(rox_prd_qty) as totqty  FROM rox_order_info WHERE rox_inv_id='$from_date' AND `rox_ord_status`!='Cancelled' GROUP BY rox_inv_id";
        $result=mysqli_query($conn,$sql1);
        return $result;
        mysqli_close($conn);
    }
    ...
    public  static function seleect_all_from_odr_info1($from_date="",$cat="",$sub="",$pro=""){
        ...
        $sql1="SELECT * FROM rox_order_info WHERE rox_inv_id='$from_date'   AND `rox_ord_status`!='Cancelled' $query  ";
...
//-------------------------------------------------------------------------------------------------------
    ...
    public static function update_invoice_delivery_details($inv_id,$rox_inv_due,$delivery_date,$delivery_time)
    {
        include("../library/dbcon.php");
        $sql="UPDATE rox_invoice SET rox_inv_due='$rox_inv_due',rox_del_date='$delivery_date',rox_inv_time='$delivery_time' WHERE rox_inv_auto_id='$inv_id'";
        $result=mysqli_query($conn,$sql);
        if($result=true)
        {
            return 1;
    ...
    public static function update_customer_delivery_details($cus_id,$delivery_address)
    {
        include("../library/dbcon.php");
        $sql="UPDATE rox_customers SET cus_address='$delivery_address' WHERE cus_id='$cus_id'";
        $result=mysqli_query($conn,$sql);
        if($result=true)
        {
            return 1;
    ...
    public static function update_order_info($order_info_id,$rox_gre_des,$rox_gre_info,$rox_gre,$rox_gre_info2)
    {
        include("../library/dbcon.php");
        $sql="UPDATE rox_order_info SET rox_gre_des='$rox_gre_des',rox_gre_info='$rox_gre_info',rox_gre='$rox_gre' ,rox_gre_info2='$rox_gre_info2' WHERE rox_line_id='$order_info_id'";
        $result=mysqli_query($conn,$sql);
        if($result=true)
        {
            return 1;
    ...
    public static function insert_cancel_invoice($inv_id="",$cancel_reason="",$cancel_by="",$date=""){

        include("../library/dbcon.php");
        $sql = "INSERT INTO rox_cancel_orders(rox_inv_id,odr_cancel_reason,odr_cancel_by,cancel_date) VALUES ('$inv_id','$cancel_reason','$cancel_by','$date')";
    ...
    public static function load_cancel_orders()
    {
        ...
        $sql5 = "SELECT co.*,adu.rox_user_name FROM rox_cancel_orders co,rox_admin_user adu where co.odr_cancel_by =adu.rox_admin_id";
    ...
    public static function update_invoice_bill_authorized($inv_id)
    {
        include("../library/dbcon.php");
        $sql="UPDATE rox_invoice SET rox_inv_authorization=1 WHERE rox_inv_auto_id='$inv_id'";
        $result=mysqli_query($conn,$sql);
        if($result=true)
        {
            return 1;
    ...
    public  static function get_invoice_all_products($inv_id){
        include("library/dbcon.php");
        $sql1="SELECT rox_line_id, rox_ord_id, rox_inv_id, rox_p_main_typ,rox_p_sub_type, rox_prd, rox_prd_val,rox_prd_price, rox_prd_qty,rox_gre_des,rox_gre,rox_gre_info, rox_gre_info2, rox_des,rox_ord_status,ord_date,GROUP_CONCAT(CONCAT_WS(' - ',rox_prd,rox_prd_qty ) separator ' <br> ') as rox_prd,sum(rox_prd_qty) as totqty  FROM rox_order_info WHERE rox_inv_id='$inv_id' AND `rox_ord_status`!='Cancelled' GROUP BY rox_inv_id";
        $result=mysqli_query($conn,$sql1);
        $row = mysqli_fetch_array($result);
        return $row;
    ...
    public  static function select_all_authorized_from_inv_limit($limit){
        include("library/dbcon.php");
        $sql1="SELECT * FROM rox_invoice where rox_inv_authorization=0 ORDER BY rox_inv_id DESC LIMIT $limit";
        $result=mysqli_query($conn,$sql1);
        return $result;
        mysqli_close($conn);
    }
    ...
    public  static function search_from_inv_limit($inv_id){
        include("library/dbcon.php");
        $sql1="SELECT i.*,c.cus_mobile,c.cus_fname FROM rox_invoice i,rox_customers c where c.cus_id=i.rox_inv_cus_id  AND (i.rox_inv_auto_id LIKE '%$inv_id%' OR c.cus_mobile LIKE '%$inv_id%' OR c.cus_fname LIKE '%$inv_id%') LIMIT 50";
        $result=mysqli_query($conn,$sql1);
        return $result;
        mysqli_close($conn);
    }

    public  static function get_invoice_payments($inv_code){
        include("library/dbcon.php");
        $sql1="SELECT * FROM rox_payment WHERE rox_inv_id='$inv_code'";
        $result=mysqli_query($conn,$sql1);
        $row = mysqli_fetch_array($result);
    ...
    public  static function select_all_cus_auto($fname){
        include("../library/dbcon.php");
        $sql1="SELECT * FROM rox_customers where cus_fname LIKE '%$fname%'";
        $result=mysqli_query($conn,$sql1);
        return $result;
        mysqli_close($conn);
    }
