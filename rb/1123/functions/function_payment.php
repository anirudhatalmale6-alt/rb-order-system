...
$val=class_payment::balance_payment($inid,$cusid,$type,$pay,$date);
$ball=$bal-$pay;
    if($ball == 0){
        $pay='0';
        $stat='0';
...
if($val==1)
{
    echo '<script type="text/javascript">window.open("../slip?inv_code='.$inid.'&paid='.$payy.'&bala='.$bal.'","newwindow","width=450,height=660");</script>';
...
if(isset($_POST['cancel_reason']))
{
    ...
    $vall=class_payment::update_status($invoid);
    $vall=class_payment::update_invoice_status($invoid);
    $vall=class_orders::insert_cancel_invoice($invoid,$cancel_reason,$cancel_by,$date);
    if($vall==1)
    {
        echo  '<script type="text/javascript">window.location="../manage_orders?status=8";</script>';
    }
    else
    {
        echo  '<script type="text/javascript">window.location="../manage_orders?status=9";</script>';
    }
}